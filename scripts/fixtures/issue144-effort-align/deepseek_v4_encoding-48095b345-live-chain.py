...
def _process_image_blocks(
    ...
    for block in blocks:
        ...
        if _is_image_block(block):
            ...
            if False and IMAGE_PLACEHOLDER in text:  # [vision-exp-hotfix] allow vLLM-inserted image placeholders
                raise ValueError(
...
def _validate_no_image_sp_tokens(msg: Dict[str, Any]) -> None:
    ...
    content = msg.get("content")
    if False and isinstance(content, str) and IMAGE_PLACEHOLDER in content:  # [vision-exp-hotfix] allow vLLM-inserted image placeholders
        raise ValueError(
            f"Message content contains image special token '{IMAGE_PLACEHOLDER}'. "
            "Images should be provided as image content blocks."
        )
    reasoning_content = msg.get("reasoning_content")
    if False and isinstance(reasoning_content, str) and IMAGE_PLACEHOLDER in reasoning_content:  # [vision-exp-hotfix] allow vLLM-inserted image placeholders
        raise ValueError(
...
# [vision-exp-hotfix] images only in user messages
# [vision-exp-hotfix] paired <image> tag (issue 165)
# [vision-exp-hotfix] tool text is not an image (issue 167)
